...
from mwr_l12l2.retrieval.monitoring import Level1Monitoring, monitor_mwr_l1
...
class TestLevel1Monitoring(unittest.TestCase):
    ...
    @patch('mwr_l12l2.retrieval.monitoring.read_mwr_summary_csv')
    @patch('glob.glob')
    def test_set_instrument_no_files(self, mock_glob, mock_read_csv):
...
class TestMonitorMwrL1Function(unittest.TestCase):
    """Test the monitor_mwr_l1 function"""
    
    def setUp(self):
        """Set up test fixtures"""
        self.test_day = dt.datetime(2023, 4, 25)
        
    @patch('mwr_l12l2.retrieval.monitoring.Level1Monitoring')
    @patch('matplotlib.pyplot.figure')
    @patch('matplotlib.pyplot.savefig')
    @patch('matplotlib.pyplot.close')
    def test_monitor_mwr_l1_no_files(self, mock_close, mock_savefig, mock_figure, mock_level1_monitoring):
        """Test monitor_mwr_l1 function when no MWR files are found"""
        # Mock the Level1Monitoring instance
        mock_instrument = Mock()
        mock_instrument.l1_instrument_list = pd.DataFrame({
            'wigos_station_id': ['0-20000-0-12345'],
            'instrument_id': ['A']
        })
        mock_instrument.wigos = '0-20000-0-12345'
        mock_instrument.inst_id = 'A'
        mock_instrument.mwr_files = []  # No MWR files
        mock_instrument.alc_files = []
        mock_instrument.inst_conf = {}
        
        mock_level1_monitoring.return_value = mock_instrument
        mock_fig = Mock()
        mock_figure.return_value = mock_fig
        
        # Run the function
        monitor_mwr_l1(self.test_day)
        
        # Verify that set_instrument and list_obs_files were called
        mock_instrument.set_instrument.assert_called_once_with('0-20000-0-12345', 'A')
        mock_instrument.list_obs_files.assert_called_once()
        
        # Verify that a figure was created for "no data" case
    ...
    def test_monitor_mwr_l1_with_files(self, mock_retrieval, mock_level1_monitoring):
        """Test monitor_mwr_l1 function when MWR files are found"""
        # Mock the Level1Monitoring instance
        mock_instrument = Mock()
        mock_instrument.l1_instrument_list = pd.DataFrame({
            'wigos_station_id': ['0-20000-0-12345'],
        ...
        mock_instrument.wigos = '0-20000-0-12345'
        mock_instrument.inst_id = 'A'
        mock_instrument.mwr_files = ['test_file.nc']  # Has MWR files
        mock_instrument.alc_files = ['alc_file.nc']
        mock_instrument.inst_conf = {'test': 'config'}
        mock_instrument.conf = {'test': 'main_config'}
        ...
        self.assertEqual(args[0], mock_instrument.conf)
    ...
    @patch('mwr_l12l2.retrieval.monitoring.Level1Monitoring')
    def test_monitor_mwr_l1_exception_handling(self, mock_level1_monitoring):
        """Test monitor_mwr_l1 function exception handling"""
        # Mock the Level1Monitoring instance to raise an exception
        mock_instrument = Mock()
        mock_instrument.l1_instrument_list = pd.DataFrame({
            'wigos_station_id': ['0-20000-0-12345'],
            'instrument_id': ['A']
        })
        mock_instrument.set_instrument.side_effect = Exception("Test exception")
    ...
    @patch('mwr_l12l2.retrieval.monitoring.Level1Monitoring')
    @patch('mwr_l12l2.retrieval.monitoring.Retrieval')
    def test_monitor_mwr_l1_multiple_instruments(self, mock_retrieval, mock_level1_monitoring):
        """Test monitor_mwr_l1 function with multiple instruments"""
        # Mock the Level1Monitoring instance with multiple instruments
        mock_instrument = Mock()
        mock_instrument.l1_instrument_list = pd.DataFrame({
            'wigos_station_id': ['0-20000-0-12345', '0-20000-0-67890'],
            'instrument_id': ['A', 'B']
        })
        mock_instrument.mwr_files = ['test_file.nc']
        mock_instrument.alc_files = ['alc_file.nc']
        mock_instrument.inst_conf = {'test': 'config'}
        mock_instrument.conf = {'test': 'main_config'}
        ...
        # Verify that set_instrument was called for each instrument
        expected_calls = [
            call('0-20000-0-12345', 'A'),
            call('0-20000-0-67890', 'B')
        ]
        mock_instrument.set_instrument.assert_has_calls(expected_calls)
...
if __name__ == '__main__':
    ...
    # Add test classes
    suite.addTest(unittest.makeSuite(TestLevel1Monitoring))
    suite.addTest(unittest.makeSuite(TestMonitorMwrL1Function))
    suite.addTest(unittest.makeSuite(TestMonitoringUtilsIntegration))

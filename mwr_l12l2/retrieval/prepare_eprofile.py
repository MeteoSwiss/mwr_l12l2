...
def main(mwr_files_in, mwr_file_out, alc_files_in=None, alc_file_out=None, delete_mwr_in=False,
    ...
    mwr = mwr.where((mwr.time > time_min) & (mwr.time < time_max))  # brackets because of precedence of & over > and <
    ...
    if alc_files_in:  # not empty list, not None
        ...
        alc = alc.where((alc.time > time_min-tolerance_alc_time) & (alc.time < time_max+tolerance_alc_time))
        alc.to_netcdf(alc_file_out)

    # TODO: return time_min, time_max whether ALC data is not empty and not NaN. CARE: cbh is not good for this as it is also NaN in case of clear sky
